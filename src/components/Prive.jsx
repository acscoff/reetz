...
export default function Prive({ profile, onBack }) {
  ...
  const [view, setView]         = useState('today') // today|week|all|overdue
  ...
  async function addTask() {
    ...
    setNewTask(''); setNewDate(''); setNewPrio(1); setNewTag(null)
    setAddOpen(false)
  }
  ...
  const active = tasks.filter(t => {
    ...
    if (view === 'overdue') return t.date < TODAY
    return true // 'all'
  }).sort((a,b) => (b.prio||1)-(a.prio||1) || a.date.localeCompare(b.date))

  const done = tasks.filter(t => t.done)

  // Progress ring pour aujourd'hui
  ...
  return (
    <div style={{ height:'100%', overflowY:'auto', background:'var(--offwhite)' }}>
      ...
      <div style={{ padding:'12px 14px 10px', display:'flex', alignItems:'center', justifyContent:'space-between', background:'var(--offwhite)', borderBottom:'1px solid var(--border)', position:'sticky', top:0, zIndex:10 }}>
        ...
        <div onClick={() => setAddOpen(!addOpen)} style={{ width:30, height:30, borderRadius:'50%', background:'var(--pink)', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer' }}>
      ...
      {/* Panel ajout */}
      {addOpen && (
        <div style={{ margin:'10px 14px 0', background:'var(--card)', border:'1.5px solid var(--border2)', borderRadius:14, padding:'12px 14px', display:'flex', flexDirection:'column', gap:8 }}>
            ...
            {Object.entries(TAGS).map(([k,v]) => (
              <div key={k} onClick={() => setNewTag(newTag===k ? null : k)}
                style={{ padding:'5px 10px', borderRadius:99, border:'1.5px solid', cursor:'pointer', fontSize:8, fontWeight:700, fontFamily:'Syne,sans-serif',
                  background: newTag===k ? v.bg : 'transparent',
                  borderColor: newTag===k ? v.color : 'var(--border2)',
                  color: newTag===k ? v.color : 'var(--muted)',
                }}>{v.label}</div>
            ))}
          </div>
      ...
      <div style={{ margin:'10px 14px 0', background:'var(--card)', border:'1px solid var(--border)', borderRadius:12, padding:'10px 12px', display:'flex', alignItems:'center', gap:10 }}>
        <div style={{ position:'relative', width:44, height:44, flexShrink:0 }}>
          <svg width="44" height="44" viewBox="0 0 44 44">
            ...
            <circle cx="22" cy="22" r="17" fill="none"
              ...
              strokeDashoffset={circ*(1-ringPct)} strokeLinecap="round"
        ...
        <div>
          ...
          <div style={{ fontSize:9, color:'var(--muted)' }}>
            {todayAll.length > 0 ? todayDone+'/'+todayAll.length+' validée'+(todayDone>1?'s':'') : 'Aucune tâche planifiée aujourd\'hui'}
          </div>
        </div>
      </div>

      {/* Filtres vue */}
      <div style={{ margin:'8px 14px 0', display:'flex', gap:5, overflowX:'auto', scrollbarWidth:'none' }}>
        {[{v:'today',l:"Aujourd'hui"},{v:'week',l:'Semaine'},{v:'all',l:'Tout'},{v:'overdue',l:'En retard'}].map(f => (
          <div key={f.v} onClick={() => setView(f.v)}
            style={{ flexShrink:0, padding:'5px 11px', borderRadius:99, fontSize:9, fontWeight:700, fontFamily:'Syne,sans-serif', cursor:'pointer', border:'1.5px solid',
              background: view===f.v ? 'var(--black)' : 'transparent',
              borderColor: view===f.v ? 'var(--black)' : 'var(--border2)',
              color: view===f.v ? 'var(--lime)' : 'var(--muted)',
            }}>{f.l}</div>
        ))}
      </div>
      ...
      <div style={{ margin:'10px 14px 0', display:'flex', alignItems:'center', gap:8 }}>
        ...
        <div style={{ fontFamily:'Syne,sans-serif', fontSize:8, fontWeight:700, letterSpacing:'.1em', textTransform:'uppercase', color:'var(--muted)' }}>
          {view==='today' ? "Aujourd'hui" : view==='week' ? 'Cette semaine' : view==='overdue' ? 'En retard' : 'Toutes'}
        </div>
        {active.length > 0 && <div style={{ fontFamily:'Syne,sans-serif', fontSize:8, fontWeight:700, color:'var(--pink)', background:'rgba(255,46,154,0.1)', borderRadius:99, padding:'2px 7px' }}>{active.length}</div>}
        <div style={{ flex:1, height:1, background:'var(--border)' }}/>
      </div>
      ...
      <div style={{ margin:'0 14px' }}>
        {active.length === 0 && (
          <div style={{ textAlign:'center', padding:'20px 0' }}>
            ...
            <div style={{ fontSize:10, color:'var(--muted)', fontStyle:'italic' }}>Rien ici — tu es à jour !</div>
          </div>
        )}
        {active.map(t => {
          const today0 = new Date(); today0.setHours(0,0,0,0)
          const taskDate = new Date(t.date); taskDate.setHours(0,0,0,0)
          const overdue = taskDate < today0
                ...
                <div style={{ display:'flex', alignItems:'center', gap:5, marginTop:3, flexWrap:'wrap' }}>
                  {t.date && (
                    <span style={{ fontSize:8, color: overdue ? 'var(--pink)' : 'var(--muted)', background: overdue ? 'rgba(255,46,154,0.1)' : 'rgba(0,0,0,0.05)', borderRadius:4, padding:'1px 5px', fontWeight: overdue ? 700 : 400 }}>
                      {overdue ? '⚠️ ' : ''}{isToday ? "Aujourd'hui" : jours[new Date(t.date).getDay()]+' '+new Date(t.date).getDate()+' '+mois[new Date(t.date).getMonth()]}
                    </span>
                  )}
                  {t.tag && TAGS[t.tag] && (
